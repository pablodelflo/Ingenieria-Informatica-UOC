...
void readCustomer(tCustomer *customer, tError *retVal) {	
	...
	*retVal = OK;
	...
	printf("Profile (1 digit being 0=FAMILIAR, 1=CONSERVATIVE, 2=ADVENTUROUS, 3=LONELY, 4=CURIOUS, 5=INNOVATIVE, 6=DARED):\n>> ");
	scanf("%d",&number);
...
{
	int i, j;
    tCustomerTable selectedCustomers;
    tHotelTable recommendedHotels;
    
    customerTable_selectCustomers(appData.customers, city, &selectedCustomers);
    
	for(i=0;i<selectedCustomers.nCustomers;i++) 
    {
        customerRecommendation(selectedCustomers.table[i], appData.hotels,
                               city, price, distance, points, &recommendedHotels );
        if (recommendedHotels.nHotels > 0) {
            printf("CUSTOMER\n");
            printCustomer( selectedCustomers.table[i], i+1);
        }
            
        for (j= 0; j < recommendedHotels.nHotels; j++) {
            printf("\t");
            printHotel( recommendedHotels.table[j], j+1 );            
        }
