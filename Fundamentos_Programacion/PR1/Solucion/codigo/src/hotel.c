...
void customerRecommendation(tCustomer customer, tHotelTable hotels, char *city, float price, float distance, int points, tHotelTable *result) 
{
/*************** PR1 EX5A *****************/
 
	char customerCity[MAX_CITY];
	char cityParameter[MAX_CITY];
	char hotelCity[MAX_CITY];
	int i=0;
	
	hotelTable_init(result);
	
	strncpy(cityParameter,city,MAX_CITY);//Ciudad entrada por parámetro para filtro de las ciudades
	strncpy(customerCity,customer.birthCity,MAX_CITY);//Ciudad del customer
	
	if (hotels.nHotels > 0){
		while (i<hotels.nHotels){
			strncpy(hotelCity,hotels.table[i].city,MAX_CITY);
			
			if (strcmp(hotelCity,customerCity)!=0){
				if (strcmp(hotelCity,cityParameter)==0){
					hotelTable_select(hotels,city,price,distance,points,result);
				}
